...
function CRI700_func(
    ::Type{TFL}, B, RE1; const1::Number=TFL(1.0), const2::Number=TFL(1.0)
) where {TFL<:Number}
...
function CRI550_func(
    ::Type{TFL}, B, G; const1::Number=TFL(1.0), const2::Number=TFL(1.0)
) where {TFL<:Number}
...
function ENDVI_func(
    ::Type{TFL}, N, G, B; const1::Number=TFL(2), const2::Number=TFL(2)
) where {TFL<:Number}
...
function sNIRvSWIR_func(
    ::Type{TFL}, N, R, S2; const1::Number=TFL(2.0), const2::Number=TFL(2.0)
) where {TFL<:Number}
...
function sNIRvNDPI_func(
    ::Type{TFL}, N, alpha, R, S2; const1::Number=TFL(1.0), const2::Number=TFL(1.0)
) where {TFL<:Number}
    return (N - (alpha * R + (const1 - alpha) * S2)) /
           (N + (alpha * R + (const2 - alpha) * S2)) * N
end
