...
@testset "Tests for linear, poly, and RBF functions" begin
    for T in types
        ...
        a_v, b_v, c_v, p_v, sigma_v = T.([1, 2, 3]), T.([4, 5, 6]), T.([1, 1, 1]), T.([2, 2, 2]), T.([5, 5, 5])
        df = DataFrame(; a=T.([1, 2]), b=T.([3, 4]), c=T.([1, 1]), p=T.([2, 2]), sigma=T.([5, 5]))
        ...
        @testset "Tests for RBF function with type $T" begin
            ...
            expected_array_result = exp.((-1.0 .* ((a_v .- b_v) .^ 2.0)) ./ (2.0 .* (sigma_v .^ 2.0)))
