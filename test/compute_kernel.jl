...
@testset "Compute Kernel Tests for $T" for T in floats
    ...
    @testset "Linear Kernel" begin
        ...
        @test compute_kernel(linear, params) == T(6)
        @test eltype(compute_kernel(linear, params)) == T

        params = Dict("a" => T.([1, 2]), "b" => T.([3, 4]))
        @test compute_kernel(linear, params) == T.([3, 8])
        @test eltype(compute_kernel(linear, params)) == T
    ...
    @testset "Polynomial Kernel" begin
        ...
        @test eltype(compute_kernel(poly, params)) == T

        params = Dict(
            "a" => T.([1, 2]), "b" => T.([2, 3]), "c" => T.([1, 1]), "p" => T.([2, 3])
        )
        @test compute_kernel(poly, params) == T.([9, 343])
        @test eltype(compute_kernel(poly, params)) == T

        params = (a=T(2), b=T(3), c=T(1), p=T(2))
        @test compute_kernel(poly, params) == T(49)
        @test eltype(compute_kernel(poly, params)) == T
    end
    ...
    @testset "RBF Kernel" begin
        ...
        @test compute_kernel(RBF, params) ≈ T(exp(-0.5))

        params = Dict("a" => T.([1, 2]), "b" => T.([2, 1]), "sigma" => T.([1, 2]))
        @test compute_kernel(RBF, params) ≈ T.([exp(-0.5), exp(-0.125)])
